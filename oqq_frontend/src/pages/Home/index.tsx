 ...
 *
 * Page d'accueil oùquandquoi.fr
 * - Affiche les activités avec filtres, favoris, carrousels, pub partenaires
 * - Mapping dynamique de isFavorite pour chaque activité selon userFavorites
 * - Jamais de donnée nominative affichée (RGPD)
 */
...
const HomePage: React.FC<HomePageProps> = ({
  ...
  // Fetch recently viewed activities if authenticated
  useEffect(() => {
  ...
  // Filter activities for various carousels/sections
  const favorites = allOnlineActivities.filter(
    (a) => userFavorites.includes(getActivityId(a)) && !isExpiredActivity(a.when)
  );

  const onlineActivities = allOnlineActivities.filter(
    (a) => !isExpiredActivity(a.when)
  );

  const filteredNotExpired = filteredResults.filter(
    (a) => !isExpiredActivity(a.when)
  );

  const nearbyActivities =
    active && lat && lon
      ? allOnlineActivities.filter(
          (a) =>
        ...
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-2xl font-semibold text-black flex items-center">
            ...
            <span className="text-lg font-normal text-gray-500 ml-2">
              ...
              {nearbyActivities.length > 1 ? "activités" : "activité"})
            </span>
          </h3>
        </div>
        <div className="mb-8">
      ...
      <>
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-2xl font-semibold text-black flex items-center">
            Activités en ligne
            <span className="text-lg font-normal text-gray-500 ml-2">
              ({onlineActivities.length}{" "}
              {onlineActivities.length > 1 ? "activités" : "activité"})
            </span>
          </h3>
        </div>
        <div className="mb-8">
  ...
  // Search filters presence
  const hasWhere =
    !!(
      whereFilter.location ||
      whereFilter.label ||
      whereFilter.lat ||
      whereFilter.lon ||
      whereFilter.distance
    );
  const hasWhen = !!whenFilter;
  const hasWhat =
    !!(value.keyword?.trim() || value.category || value.subcategory);
            ...
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-2xl font-semibold text-black flex items-center">
                ...
                <span className="text-lg font-normal text-gray-500 ml-2">
                  ...
                  {recentlyViewed.length > 1 ? "activités" : "activité"})
                </span>
              </h3>
            </div>
            <Carousel>
